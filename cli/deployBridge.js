 ...
 const SGAS_LIMIT = Number(process.env.SGAS_LIMIT),
     SGAS_PRICE = Number(process.env.SGAS_PRICE);
 const DGAS_LIMIT = Number(process.env.DGAS_LIMIT),
     DGAS_PRICE = Number(process.env.DGAS_PRICE);
 ...
 function createChainConfig(chainName, chainId, bridgeAddress, erc20handlerAddress, endpoint = '', relayerAddress = '') {
     let chainConfig = {
         ...
         opts: {
             ...
             maxGasPrice: GAS_PRICE.toString()
         }
     }
